...
(function () {
  ...
  // EQ Visualizer — exposed globally for late init
  window._eqMuted = function () { return isMuted; };
...
(function () {
  ...
  var data = [
    ...
    { year: '2012', tag: 'FRACTURE', major: true, density: 1,
      text: 'Everything stopped. A loss that rewrote the operating system. The kind of silence that follows when the ground beneath you disappears. No strategy for this. No framework. Just the raw confrontation with what matters and what never did.',
      meta: 'LOSS · SILENCE · RECALIBRATION' },
    { year: '2013', tag: 'STILL', density: 1,
      text: 'The year of standing still. Not recovery — not yet. Just the slow process of learning to breathe again. Redefining what work means when meaning itself has shifted. Everything before feels like a different person.',
      meta: 'GRIEF · RECONSTRUCTION · MEANING' },
    { year: '2014', tag: 'RESURGENCE', major: true, density: 7,
      text: 'Rising again. Not the same — deeper. Consulting practice founded from a place of clarity that only comes after breaking. Real structural change, not performance. Every engagement now carries a weight that wasn\'t there before.',
      meta: 'CONSULTING · RESURGENCE · DEPTH' },
  ...
  var yearConcepts = {
    ...
    '2009': 'foundation', '2010': 'structure',
    '2012': 'silence', '2013': 'endurance', '2014': 'resurgence',
  ...
  var globalEvents = {
    ...
    '2012': ['SpaceX Dragon ISS', 'Higgs Boson Found', 'Curiosity Mars Landing', 'Tesla Model S', 'Raspberry Pi Launch', 'Google Glass Preview', 'Deep Learning Breakout', 'CRISPR Discovery', 'WhatsApp 100M Users', 'Sandy Hook'],
    '2013': ['Snowden Revelations', 'Bitcoin $1000', 'Boston Marathon', 'Pope Francis', 'PS4 Launch', 'Vine & Snapchat Growth', 'Boston Dynamics Atlas', 'Gravity Film', '3D Printing Hype', 'Mandela Passing'],
...
(function () {
  ...
  var levels = [];
  var targets = [];
  for (var i = 0; i < cols; i++) { levels[i] = 0; targets[i] = 0; }
  var t = 0;
  ...
  function draw() {
    ...
    ctx.clearRect(0, 0, w, h);
    t += 0.016;
    var playing = window._eqMuted ? !window._eqMuted() : false;

    for (var col = 0; col < cols; col++) {
      // Generate organic movement
      if (playing) {
        if (Math.random() < 0.15) {
          targets[col] = Math.random() * 0.5 + 0.15 +
            Math.sin(t * (1.5 + col * 0.3)) * 0.25 +
            Math.sin(t * 0.7 + col) * 0.15;
      ...
      var active = Math.ceil(levels[col] * rows);
      ...
      for (var row = 0; row < rows; row++) {
        ...
        if (on) {
          var pct = row / rows;
          if (pct > 0.7) {
            ctx.fillStyle = 'rgba(255,87,90,' + (0.7 + pct * 0.3) + ')';
            ctx.shadowColor = 'rgba(255,87,90,0.5)';
            ctx.shadowBlur = 5;
          } else if (pct > 0.35) {
            ctx.fillStyle = 'rgba(255,87,90,' + (0.35 + pct * 0.35) + ')';
            ctx.shadowBlur = 0;
          } else {
            ctx.fillStyle = 'rgba(249,248,242,' + (0.25 + pct * 0.4) + ')';
            ctx.shadowBlur = 0;
          }
        } else {
